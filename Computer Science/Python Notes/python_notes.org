...
5
#+END_EXAMPLE
The above function call is similar as: ~func(1, 2, 3, 4, 5)~. And ~func()~ has been defined to take a ~*nums~ argument. So each element in ~arg~ has been passed in ~func()~ separately. If we remove ~*~ when calling function, we have:
#+begin_src python -n
def func(*nums):
    for num in nums:
        print(num)
